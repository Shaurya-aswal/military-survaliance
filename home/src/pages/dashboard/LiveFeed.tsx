      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Live Detection Feed</h1>
        ...
        {analyses.length > 0 && (
          <div className="grid grid-cols-3 gap-3">
            <div className="flex items-center gap-3 rounded-xl border border-[hsl(217,33%,17%)]/60 bg-[hsl(222,47%,8%)]/50 p-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-500/10">
                <Eye className="h-4 w-4 text-blue-400" />
              </div>
              <div>
                <p className="text-lg font-bold text-slate-100 font-mono">{totalDetections}</p>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Detections</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-xl border border-[hsl(217,33%,17%)]/60 bg-[hsl(222,47%,8%)]/50 p-3">
              ...
              <div>
                <p className="text-lg font-bold text-slate-100 font-mono">{totalThreats}</p>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Threats</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-xl border border-[hsl(217,33%,17%)]/60 bg-[hsl(222,47%,8%)]/50 p-3">
              ...
              <div>
                <p className="text-lg font-bold text-slate-100 font-mono">{analyses.length}</p>
                <p className="text-[10px] text-slate-500 uppercase tracking-wider">Scans</p>
              </div>
            </div>
          </div>
        )}
      ...
      <CardHeader className="pb-3 pt-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <div className={cn(
              'flex h-11 w-11 items-center justify-center rounded-xl flex-shrink-0 border transition-colors',
              hasThreat
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <Badge variant="outline" className="text-[10px] font-mono text-slate-300 border-slate-600/60 bg-[hsl(217,33%,17%)]/30">
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-600 transition-all hover:bg-red-500/10 hover:text-red-400"
          ...
          {analysis.detections.map((detection, idx) => {
            ...
            return (
              <div
                ...
                className={cn(
                  'flex items-center gap-4 rounded-xl border p-3 cursor-pointer transition-all duration-200',
                  'hover:bg-[hsl(217,33%,17%)]/30 hover:border-blue-500/25 hover:translate-x-0.5',
                  cfg.border, cfg.bg
                )}
              >
                <span className="text-[10px] text-slate-600 font-mono w-5 text-center flex-shrink-0">
                  #{idx + 1}
                </span>
                ...
                <div className="flex items-center gap-1.5 text-xs text-slate-400 font-mono flex-shrink-0 tabular-nums">
                ...
                <Badge
                  variant="outline"
                  className={cn('text-[10px] font-mono flex-shrink-0', cfg.color, cfg.border, cfg.bg)}
                >
                  {cfg.label}
                </Badge>

                <ChevronRight className="w-4 h-4 text-slate-600 flex-shrink-0 transition-transform group-hover:translate-x-0.5" />
              </div>
            );
          })}
        </div>
